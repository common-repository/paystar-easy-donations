...
class Easy_Donationns_Admin_Payments_List extends EDT_WP_List_Table {
	...
	function usort_reorder( $a, $b ) {
		...
		$order = ( ! empty($_GET['order'] ) ) ? sanitize_text_field($_GET['order']) : 'dec';
	...
	function process_bulk_action() {
		...
		if( 'delete' === $this->current_action() ) {
			...
			if( isset( $_POST['payments'] ) && is_array( $_POST['payments'] ) ){
				$_POST_payments = array();
				foreach($_POST['payments'] as $n => $v) {
					$_POST_payments[$n] = sanitize_text_field($v);
				}
				$payments_for_delete = edt_ins()->payment->get_payments_by_id_arr( $_POST_payments );
                edt_ins()->payment->delete_payments( $payments_for_delete );
				//$afcSelectors->updateElems( 'remove', $delSelectors );
				$this->type = 'updated';
				$this->message = count( $_POST_payments ) . __( ' Payment Log\'s has been deleted. ', EDT_TEXT_DOMAIN );
			}
			else{
				$this->type = 'error';
    ...
    function beautify_results( $arr ) {
        ...
        if( is_array( $arr ) ) {
            foreach( $arr as $key ) {
                ...
                $key['details'] = sprintf('<a href="?page=%s&action=%s&payment_id=%d">'. __( 'View Details', EDT_TEXT_DOMAIN ) . '</a>', sanitize_text_field($_REQUEST['page']), 'show_info', $key['id'] );
                $temp[] = $key;
            }
        }
	...
	function show_message() {
		if($this->message != null && $this->type != null )
			echo '<div id="edt-setting-error" class="' . esc_html($this->type) . ' settings-error"><p><strong>' . esc_html($this->message) . '</strong></p></div>';
	}
}