...
class Easy_Donations_Gateway {
    ...
    public function __construct() {
        if( isset( $_GET['listener'] ) && sanitize_text_field($_GET['listener']) == 'paystar-easy-donations' ){
            if( ! isset( $_POST['order_id'] ) || ! is_numeric( sanitize_text_field($_POST['order_id']) ) )
                wp_die( __( 'Returned Payment ID was invalid.' , EDT_TEXT_DOMAIN ) );
            if (!(isset($_SESSION['paystar_edt_id']) && sanitize_text_field($_SESSION['paystar_edt_id']) == sanitize_text_field($_POST['order_id']))) {
                session_destroy();
                session_id(sanitize_text_field($_GET['shf_key']));
                session_start();
            }
            $this->posted_data = array();
            foreach($_POST as $n => $v) {
                $this->posted_data[$n] = sanitize_text_field($v);
            }
    ...
    public function do_on_return() {
        ...
        $temp = edt_ins()->payment->get_payment_by_id( sanitize_text_field($_POST['order_id']) );
        $temp = $temp[0];
        if( ! isset( $temp['id'] ) )
